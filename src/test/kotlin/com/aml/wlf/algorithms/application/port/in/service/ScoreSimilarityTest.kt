...
class ScoreSimilarityTest {
    ...
    fun findMostSimilarName_low() {
        ...
        Mockito.`when`(nameSimilarityUsecase.calculate("John Doe", "Jane Doe")).thenReturn(0.3)
    ...
    fun findMostSimilarName() {
        ...
        Mockito.`when`(fetchWatchlistUsecase.fetch("BGD", LocalDate.of(1990, 1, 1))).thenReturn(watchlist)
        Mockito.`when`(nameSimilarityUsecase.calculate("John Doe", "John Doe")).thenReturn(1.0)
